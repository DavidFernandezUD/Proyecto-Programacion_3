...
public class ChestScreen implements Drawable {
	...
	private boolean onCloseButton = false;
	...
	/**
	 * Updates the inventory in response to user input, and triggers sounds when
	 * necessary.
	 */
	public void update() {
		if (gamePanel.keyHandler.isKeyToggled(KeyEvent.VK_W) != upToggled) {
			upToggled = !upToggled;
			if (slotRow == 0 && cursorOnChest) {
				onCloseButton = true;
			} else {
				if (slotRow != 0) {
					slotRow--;
		...
		} else if (cursorOnChest) {
			if (gamePanel.keyHandler.isKeyToggled(KeyEvent.VK_ENTER) != enterToggled) {
				enterToggled = !enterToggled;
				selectItemFromChest();
			}
		} else if (cursorOnInventory) {
			if (gamePanel.keyHandler.isKeyToggled(KeyEvent.VK_ENTER) != enterToggled) {
				enterToggled = !enterToggled;
	...
	public void draw(Graphics2D g2) {
		...
		if (onCloseButton) {
			g2.drawRoundRect(cFrameX + 18, cFrameY + 18, cFrameWidth - 40, cFrameHeight - 40, 10, 10);
		}
	}

	/**
	 * Helper method that flattens row col indexes into a single index from
	 * inventory.
	 * 
	...
	/**
	 * Helper method that flattens row col indexes into a single index from chest.
	 * 
	 * @return Flattened index.
	 */
